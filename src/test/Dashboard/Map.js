...
function Map() {
  ...
  useEffect(() => {
    const selectedDateTime = `${selectedYear}-${selectedMonth}-${selectedDay} ${selectedHour}:${String(currentMinute).padStart(2, '0')}`;
    const filteredData = routes.filter((route) => route.date.startsWith(selectedDateTime));
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      <div style={{
        ...
        <select value={selectedYear} onChange={(e) => setSelectedYear(e.target.value)} style={{ padding: '5px', fontSize: '14px' }}>
          {years.map((year) => <option key={year} value={year}>{year}</option>)}
        </select>

        <select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} style={{ padding: '5px', fontSize: '14px' }}>
          {months.map((month) => <option key={month} value={month}>{month}</option>)}
        </select>

        <select value={selectedDay} onChange={(e) => setSelectedDay(e.target.value)} style={{ padding: '5px', fontSize: '14px' }}>
          {days.map((day) => <option key={day} value={day}>{day}</option>)}
        </select>

        <select value={selectedHour} onChange={(e) => setSelectedHour(e.target.value)} style={{ padding: '5px', fontSize: '14px' }}>
          {hours.map((hour) => <option key={hour} value={hour}>{hour}</option>)}
        </select>

        <select value={selectedMinute} onChange={(e) => setSelectedMinute(e.target.value)} style={{ padding: '5px', fontSize: '14px' }}>
      ...
      {/* 지도 */}
      <MapContainer center={[37.5509, 127.0738]} zoom={15} style={{ height: '100vh', width: '100%' }}>
